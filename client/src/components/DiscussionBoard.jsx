            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                ...
                <form onSubmit={handlePostQuestion} className="space-y-4">
                    ...
                    <textarea
                        ...
                        rows={3}
                        className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none"
                    />
            ...
            <div className="space-y-4">
                {discussions.length === 0 ? (
                    ...
                    discussions.map(discussion => (
                        <div key={discussion.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                            <div className="p-6 cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => setExpandedDiscussion(expandedDiscussion === discussion.id ? null : discussion.id)}>
                                <div className="flex justify-between items-start mb-2">
                                    <h4 className="text-lg font-semibold text-slate-900">{discussion.title}</h4>
                                ...
                                <div className="bg-slate-50 p-6 border-t border-slate-200">
                                        ...
                                        {discussion.replies.map(reply => (
                                            <div key={reply.id} className="flex gap-3">
                                                <div className="flex-shrink-0">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${reply.user.role === 'TEACHER' ? 'bg-blue-100 text-blue-600' : 'bg-slate-200 text-slate-600'
                                                        }`}>
                                                ...
                                                <div className="flex-1 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                                                    <div className="flex items-center justify-between mb-1">
                                                        <span className={`text-sm font-semibold ${reply.user.role === 'TEACHER' ? 'text-blue-700' : 'text-slate-900'
                                                            }`}>
                                    ...
                                    <div className="flex gap-3">
                                        <textarea
                                            ...
                                            className="flex-1 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none"
                                        />
                                        <button
                                            onClick={() => handlePostReply(discussion.id)}
                                            disabled={!replyContent[discussion.id]?.trim()}
                                            className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary/90 disabled:opacity-50 self-end"
                                        >
                                            Reply
                                        </button>
                                    </div>
