 ...
 * Validates that all mock data has been removed and real-time data is working
 */
...
try {
  ...
  for (const filePath of filesToCheck) {
    if (fs.existsSync(filePath)) {
      ...
      // Simple string searches to avoid regex issues
      const patterns = [];
      if (content.includes('Math.random()')) {
        patterns.push('Math.random()');
      }
      if (content.includes('mockData') || content.includes('MOCK_DATA')) {
        patterns.push('mock data');
      }
      
      if (patterns.length > 0) {
        mockDataFound = true;
        results.mockDataRemoval.details.push(`❌ Mock data found in ${path.basename(filePath)}: ${patterns.join(', ')}`);
      } else {
